...
"""
# admin_panel.py
# Modul für den Admin Panel Tab (F)
...
_DEFAULT_FEED_IN_TARIFFS_FALLBACK: Dict[str, List[Dict[str, Any]]] = {
    ...
    "full": [{"kwp_min": 0.0, "kwp_max": 10.0, "ct_per_kwh": 12.9},
             ...
             {"kwp_min": 40.01, "kwp_max": 100.0, "ct_per_kwh": 10.8}] 
}
...
def get_text_local(key: str, fallback_text: str) -> str:
    """
    Lokale Hilfsfunktion zum Abrufen von Texten für das Admin-Panel.
    Greift auf `st.session_state['_admin_panel_texts']` zu, das in `render_admin_panel`
    garantiert als Dictionary gesetzt wird.
    """
    admin_texts_dict = st.session_state.get('_admin_panel_texts', {}) # Fallback auf leeres Dict
    # Stelle sicher, dass admin_texts_dict wirklich ein dict ist, bevor .get() aufgerufen wird
    if not isinstance(admin_texts_dict, dict):
        # Dieser Fall sollte durch die Korrektur in render_admin_panel nicht mehr eintreten,
        # aber als zusätzliche Sicherheitsmaßnahme.
        # print(f"WARNUNG: get_text_local - st.session_state['_admin_panel_texts'] ist kein Dict, sondern {type(admin_texts_dict)}. Fallback auf leeres Dict.")
        admin_texts_dict = {}
...
def render_company_crud_tab(
    # KORREKTUR: `texts` Parameter entfernt, da `get_text_local` verwendet wird.
    ...
    if active_company_id_str is not None:
        try: active_company_id = int(active_company_id_str)
        except (ValueError, TypeError): st.warning(f"Ungültige active_company_id '{active_company_id_str}' in Admin-Settings.")
    ...
    with st.expander(expander_title, expanded=expander_expanded_default):
        form_key_company_ui_crud = f"company_form_ui_crud_{st.session_state.editing_company_id or 'new_company_v15_final'}"
        with st.form(key=form_key_company_ui_crud, clear_on_submit=False): 
            ...
            if current_logo_b64_form_crud and not uploaded_logo_file_ui_crud and st.session_state.editing_company_id:
                try: st.image(base64.b64decode(current_logo_b64_form_crud), caption=get_text_local("admin_current_logo_caption","Aktuelles Logo"), width=100) # Text-Key
        ...
        if submitted_company_form_btn_crud:
            ...
            if not company_name_stripped_crud: 
                ...
                data_for_db_crud = { "name": company_name_stripped_crud, "street": c_street.strip() if isinstance(c_street, str) else None, "zip_code": c_zip.strip() if isinstance(c_zip, str) else None, "city": c_city.strip() if isinstance(c_city, str) else None, "phone": c_phone.strip() if isinstance(c_phone, str) else None, "email": c_email.strip() if isinstance(c_email, str) else None, "website": c_web.strip() if isinstance(c_web, str) else None, "tax_id": c_tax.strip() if isinstance(c_tax, str) else None, "commercial_register": c_reg.strip() if isinstance(c_reg, str) else None, "bank_details": c_bank.strip() if isinstance(c_bank, str) else None, "pdf_footer_text": c_footer.strip() if isinstance(c_footer, str) else None }
                if uploaded_logo_file_ui_crud is not None:
                    if uploaded_logo_file_ui_crud.size > 2 * 1024 * 1024: 
                        st.error(get_text_local("admin_error_logo_too_large","Logo-Datei ist zu groß (max. 2MB). Altes Logo wird beibehalten, falls vorhanden.")) # Text-Key
                ...
                if st.session_state.editing_company_id:
                    ...
                    if new_company_id_crud:
                        st.success(get_text_local("admin_company_added_success_param", f"Firma '{company_name_stripped_crud}' erfolgreich angelegt.").format(company_name_stripped_crud=company_name_stripped_crud)) # Text-Key
                        if len(db_list_companies_func()) == 1: 
                            if db_set_default_company_func(new_company_id_crud):
                                st.info(get_text_local("admin_info_company_set_default_param", f"Firma '{company_name_stripped_crud}' wurde als Standard und aktive Firma gesetzt.").format(company_name_stripped_crud=company_name_stripped_crud)) # Text-Key
                            else:
                                st.warning(get_text_local("admin_warning_company_set_default_failed","Konnte Firma nicht automatisch als Standard/Aktiv setzen.")) # Text-Key
                        st.session_state.editing_company_id = None 
                        st.session_state.selected_page_key_sui = "admin" 
                        st.rerun()
                    else:
        ...
        if st.session_state.editing_company_id: 
            if st.button(get_text_local("admin_finish_edit_company_button", "Bearbeitung abschließen / Neue Firma anlegen"), key=f"finish_edit_company_btn_ui_crud_{st.session_state.editing_company_id}_v15_final"):
                st.session_state.editing_company_id = None
                st.session_state.selected_page_key_sui = "admin" 
                st.rerun()
            ...
            form_key_doc_upload_crud = f"company_document_upload_form_crud_{current_company_id_for_docs_crud}_v15_final"
            with st.form(key=form_key_doc_upload_crud, clear_on_submit=True):
                st.markdown("**"+get_text_local("admin_upload_new_document_header","Neues Dokument hochladen:")+"**") # Text-Key
                ...
                if submitted_doc_upload_btn_crud:
                    ...
                    if not doc_display_name_val_crud:
                        st.error(get_text_local("admin_error_doc_display_name_required","Bitte einen Anzeigenamen für das Dokument eingeben.")) # Text-Key
                    elif not uploaded_pdf_file_doc_crud:
                        st.error(get_text_local("admin_error_doc_file_required","Bitte eine PDF-Datei für das Dokument auswählen.")) # Text-Key
                    elif uploaded_pdf_file_doc_crud.size > 5 * 1024 * 1024: 
                        st.error(get_text_local("admin_error_doc_file_too_large","Dokument-Datei ist zu groß (max. 5MB).")) # Text-Key
                    else:
                        ...
                        if doc_id_db_crud:
                            st.success(get_text_local("admin_success_doc_uploaded_param","Dokument '{doc_name}' erfolgreich hochgeladen.").format(doc_name=doc_display_name_val_crud)) # Text-Key
                            st.session_state.selected_page_key_sui = "admin" 
                            st.rerun() 
                        else:
                            st.error(get_text_local("admin_error_doc_saving_to_db","Fehler beim Speichern des Dokuments in der Datenbank.")) # Text-Key
            ...
            if company_documents_list_crud:
                st.markdown("**"+get_text_local("admin_existing_documents_header","Vorhandene Dokumente:")+"**") # Text-Key
                for doc_item_crud in company_documents_list_crud:
                    ...
                    delete_doc_btn_key_list_crud = f"delete_company_doc_btn_crud_{doc_id_list_crud}_v15_final"
                    confirm_delete_doc_session_key_crud = f"confirm_delete_company_doc_sess_crud_{doc_id_list_crud}_v15_final"
                    ...
                    if cols_doc_display_crud[3].button("🗑️", key=delete_doc_btn_key_list_crud, help="Dokument löschen"):
                        if st.session_state.get(confirm_delete_doc_session_key_crud, False): 
                            if db_delete_company_document_func(doc_id_list_crud):
                                st.success(get_text_local("admin_success_doc_deleted_param","Dokument '{doc_name}' gelöscht.").format(doc_name=doc_item_crud.get('display_name'))) # Text-Key
                                if confirm_delete_doc_session_key_crud in st.session_state:
                                    del st.session_state[confirm_delete_doc_session_key_crud]
                                st.session_state.selected_page_key_sui = "admin" 
                                st.rerun()
                            else:
                                st.error(get_text_local("admin_error_deleting_doc_param","Fehler beim Löschen des Dokuments '{doc_name}'.").format(doc_name=doc_item_crud.get('display_name'))) # Text-Key
                        else: 
                            st.session_state[confirm_delete_doc_session_key_crud] = True
                            st.warning(get_text_local("admin_warning_confirm_delete_doc_param","Sicher, dass Dokument '{doc_name}' gelöscht werden soll? Erneut 'Löschen' klicken.").format(doc_name=doc_item_crud.get('display_name'))) # Text-Key
                            st.session_state.selected_page_key_sui = "admin" 
                            st.rerun() 
                st.markdown("---")
            else:
                st.caption(get_text_local("admin_no_documents_for_company_yet","Noch keine Dokumente für diese Firma hochgeladen.")) # Text-Key
    ...
    if active_company_id_for_list_display_str_crud is not None:
        try: active_company_id_for_list_display_crud = int(active_company_id_for_list_display_str_crud)
        except (ValueError, TypeError): active_company_id_for_list_display_crud = None
    ...
    if not all_companies_list_crud:
        ...
        column_headers_crud = ["ID", "Firmenname", "Logo", "Standard", "Aktiv", "📝", "🗑️", "Als Standard/Aktiv"]
        ...
        for company_list_item_crud in all_companies_list_crud:
            ...
            if company_logo_b64_list_view_crud:
                try: row_cols_crud[2].image(base64.b64decode(company_logo_b64_list_view_crud), width=40)
                except Exception: row_cols_crud[2].caption("err")
            else: row_cols_crud[2].caption("-")
            is_db_default_list_crud = company_list_item_crud.get('is_default') == 1
            row_cols_crud[3].write("✅" if is_db_default_list_crud else "❌")
            is_currently_active_item_crud = (active_company_id_for_list_display_crud is not None and company_id_item_int_crud is not None and active_company_id_for_list_display_crud == company_id_item_int_crud)
            row_cols_crud[4].write("✅" if is_currently_active_item_crud else "❌")
            if company_id_item_int_crud is not None:
                if row_cols_crud[5].button("📝", key=f"edit_company_list_btn_crud_{company_id_item_int_crud}_v15_final", help="Bearbeiten"):
                    st.session_state.editing_company_id = company_id_item_int_crud; st.session_state.selected_page_key_sui = "admin"; st.rerun() 
                confirm_del_key_list_item_crud = f"confirm_delete_company_list_item_crud_{company_id_item_int_crud}_v15_final"
                if row_cols_crud[6].button("🗑️", key=f"delete_company_list_btn_crud_{company_id_item_int_crud}_v15_final", help="Löschen"):
                    if st.session_state.get(confirm_del_key_list_item_crud, False): 
                        if db_delete_company_func(company_id_item_int_crud):
                            st.success(get_text_local("admin_success_company_deleted_param","Firma '{company_name}' gelöscht.").format(company_name=company_list_item_crud.get('name'))) #Text-Key
                            if confirm_del_key_list_item_crud in st.session_state: del st.session_state[confirm_del_key_list_item_crud]
                            if st.session_state.editing_company_id == company_id_item_int_crud: st.session_state.editing_company_id = None
                            st.session_state.selected_page_key_sui = "admin"; st.rerun() 
                        else: st.error(get_text_local("admin_error_deleting_company_param","Fehler Löschen Firma '{company_name}'.").format(company_name=company_list_item_crud.get('name'))) # Text-Key
                    else: 
                        st.session_state[confirm_del_key_list_item_crud] = True
                        st.warning(get_text_local("admin_warning_confirm_delete_company_param","Sicher Firma '{company_name}' löschen? Erneut klicken.").format(company_name=company_list_item_crud.get('name'))) # Text-Key
                        st.session_state.selected_page_key_sui = "admin"; st.rerun() 
                if not is_currently_active_item_crud or not is_db_default_list_crud : 
                    if row_cols_crud[7].button(get_text_local("admin_set_company_default_button", "Als Standard/Aktiv"), key=f"set_default_list_btn_crud_{company_id_item_int_crud}_v15_final"):
                        if db_set_default_company_func(company_id_item_int_crud): 
                            st.success(get_text_local("admin_success_company_set_default_param","Firma '{company_name}' als Standard/Aktiv gesetzt.").format(company_name=company_list_item_crud.get('name'))) # Text-Key
                            st.session_state.selected_page_key_sui = "admin"; st.rerun() 
                        else: st.error(get_text_local("admin_error_setting_default_company","Fehler Setzen Standardfirma.")) # Text-Key
                else: row_cols_crud[7].markdown("*(Std/Aktiv)*")
            else: 
...
def render_product_management(
    ...
    if not os.path.exists(PRODUCT_DATASHEETS_BASE_DIR_ADMIN):
        try: os.makedirs(PRODUCT_DATASHEETS_BASE_DIR_ADMIN)
        except OSError as e: st.error(f"Fehler Erstellen Verzeichnis '{PRODUCT_DATASHEETS_BASE_DIR_ADMIN}': {e}")

    st.markdown("---")
    st.subheader(get_text_local("admin_product_db_upload_header", "Produktdatenbank hochladen/aktualisieren (Excel/CSV)"))
    st.info(get_text_local("admin_product_db_upload_info_v2", "Laden Sie hier eine Excel (.xlsx) oder CSV (.csv) Datei hoch..."))

    uploaded_product_file_bulk = st.file_uploader(
        get_text_local("admin_upload_product_file_label", "Produkt-Datei auswählen"),
        type=["xlsx", "csv"],
        key="product_bulk_upload_widget_v15_final" 
    )

    if uploaded_product_file_bulk is not None:
        if st.button(get_text_local("admin_process_product_file_button", "Hochgeladene Produkt-Datei verarbeiten"), key="process_bulk_product_btn_v15_final"):
            try:
                ...
                if uploaded_product_file_bulk.name.endswith('.xlsx'):
                    ...
                    for enc in encodings_to_try:
                        try:
                            df_temp = pd.read_csv(io.BytesIO(csv_bytes_content), sep=None, decimal='.', encoding=enc, engine='python', thousands=None, comment='#', skipinitialspace=True)
                            if df_temp is not None and not df_temp.empty and len(df_temp.columns) > 1 and all(isinstance(c, str) for c in df_temp.columns):
                                 st.info(f"CSV erfolgreich mit Kodierung '{enc}' und automatischem Trennzeichen gelesen.")
                                 df_products_import = df_temp; detected_encoding = enc; break
                    ...
                    if not detected_encoding: st.error("Konnte CSV-Datei mit gängigen Kodierungen/Trennzeichen nicht verarbeiten.")
                
                if df_products_import is None: st.error("Konnte die Datei nicht als Excel oder CSV einlesen oder CSV-Verarbeitung fehlgeschlagen.")
                else: 
                    df_products_import.columns = [str(col).strip().lower().replace(' ', '_') for col in df_products_import.columns]
                    st.write("Vorschau der ersten Zeilen der importierten Datei (nach Spaltennormalisierung):", df_products_import.head())
                    imported_count, updated_count, skipped_count = 0, 0, 0; error_rows = []
                    db_columns_from_product_db = ['id', 'category', 'model_name', 'brand', 'price_euro', 'capacity_w', 'storage_power_kw', 'power_kw', 'max_cycles', 'warranty_years', 'length_m', 'width_m', 'weight_kg', 'efficiency_percent', 'origin_country', 'description', 'pros', 'cons', 'rating', 'image_base64', 'datasheet_link_db_path', 'additional_cost_netto', 'updated_at'] 
                    df_products_import.rename(columns={'last_updated': 'updated_at', 'created_at': 'created_at_excel'}, inplace=True, errors='ignore')
                    ...
                    if missing_required: st.error(f"Fehlende Pflichtspalten in der hochgeladenen Datei: {', '.join(missing_required)}.")
                    else:
                        for index, row in df_products_import.iterrows():
                            product_data_import = {}; 
                            try:
                                product_data_import = {k: (v if pd.notna(v) else None) for k, v in row.to_dict().items()}
                                product_data_import_filtered = {k: v for k,v in product_data_import.items() if k in db_columns_from_product_db}
                                if not product_data_import_filtered.get('model_name') or not product_data_import_filtered.get('category'):
                                    skipped_count += 1; error_rows.append({'row': index + 2, 'reason': "Modellname oder Kategorie fehlt."}); continue
                                for num_col in ['price_euro', 'capacity_w', 'storage_power_kw', 'power_kw', 'warranty_years', 'length_m', 'width_m', 'weight_kg', 'efficiency_percent', 'additional_cost_netto', 'max_cycles', 'rating']:
                                    if num_col in product_data_import_filtered and product_data_import_filtered[num_col] is not None:
                                        try:
                                            ...
                                            if num_col in ['warranty_years', 'max_cycles']: product_data_import_filtered[num_col] = int(product_data_import_filtered[num_col])
                                        except (ValueError, TypeError): product_data_import_filtered[num_col] = None
                                existing_product = get_product_by_model_name_func(str(product_data_import_filtered['model_name']))
                                if existing_product:
                                    if update_product_func(existing_product['id'], product_data_import_filtered): updated_count += 1
                                    else: skipped_count += 1; error_rows.append({'row': index + 2, 'id': product_data_import_filtered.get('model_name'), 'reason': 'Update DB Fehler'})
                                else:
                                    if add_product_func(product_data_import_filtered): imported_count += 1
                                    else: skipped_count += 1; error_rows.append({'row': index + 2, 'id': product_data_import_filtered.get('model_name'), 'reason': 'Add DB Fehler'})
                            except Exception as e_row:
                                skipped_count += 1; current_model_name_for_error = product_data_import.get('model_name', f"Zeile {index+2}"); error_rows.append({'row': index + 2, 'model': current_model_name_for_error, 'reason': str(e_row)})
                        st.success(f"Produktimport: {imported_count} neu, {updated_count} aktual., {skipped_count} Fehler/übersprungen.");
                        if error_rows: st.warning("Fehlerhafte/übersprungene Zeilen (max. 10):"); st.json(error_rows[:10])
                        st.session_state.selected_page_key_sui = "admin"; st.rerun()
            except Exception as e_bulk_import:
                st.error(f"Fehler beim Verarbeiten der Produkt-Datei: {e_bulk_import}"); traceback.print_exc()
    st.markdown("---")

    if 'product_to_edit_id_manual' not in st.session_state: st.session_state.product_to_edit_id_manual = None
    product_categories_manual_list = list_product_categories_func()
    if not product_categories_manual_list : product_categories_manual_list = ["Modul", "Wechselrichter", "Batteriespeicher", "Wallbox", "Zubehör", "Sonstiges"]
    product_data_for_manual_form = {}; form_manual_title = get_text_local("admin_add_new_product_header_manual", "Neues Produkt manuell anlegen / Bearbeiten")
    ...
    if st.session_state.product_to_edit_id_manual:
        ...
        if product_to_edit_manual_data:
            ...
            st.error(f"Produkt mit ID {st.session_state.product_to_edit_id_manual} nicht gefunden."); st.session_state.product_to_edit_id_manual = None
    ...
    with st.expander(form_manual_title, expanded=(st.session_state.product_to_edit_id_manual is not None or not list_products_func(None))):
        form_key_manual_prod_ui = f"product_form_manual_ui_man_{st.session_state.product_to_edit_id_manual or 'new_prod_man_v15_final'}" 
        with st.form(key=form_key_manual_prod_ui, clear_on_submit=False):
            ...
            default_cat_idx = 0; current_cat_val = product_data_for_manual_form.get('category')
            if current_cat_val and current_cat_val in available_cats_form: default_cat_idx = available_cats_form.index(current_cat_val)
            ...
            p_price_form = st.number_input(label=get_text_local("product_price_label","Preis (€)"), min_value=0.0, value=float(product_data_for_manual_form.get('price_euro', 0.0)), step=0.01, format="%.2f", key=f"{form_key_manual_prod_ui}_price_man")
            p_add_cost_form = st.number_input(label=get_text_local("product_additional_cost_label","Zusatzkosten Netto (€)"), min_value=0.0, value=float(product_data_for_manual_form.get('additional_cost_netto', 0.0)),step=0.01,format="%.2f", key=f"{form_key_manual_prod_ui}_add_cost_man")
            p_warranty_form = st.number_input(label=get_text_local("product_warranty_label","Garantie (Jahre)"), min_value=0, value=int(product_data_for_manual_form.get('warranty_years', 0)), step=1, key=f"{form_key_manual_prod_ui}_warranty_man")
            ...
            if current_product_image_b64_form and not uploaded_product_image_manual_file_form and st.session_state.product_to_edit_id_manual:
                try: st.image(base64.b64decode(current_product_image_b64_form), caption=get_text_local("product_current_image_caption","Aktuelles Produktbild"), width=100)
                except Exception: pass 
            ...
            if current_datasheet_link: st.caption(f"{get_text_local('product_current_datasheet_caption','Aktuelles Datenblatt')}: `{current_datasheet_link}`")
            ...
            if p_category_form == 'Modul':
                p_capacity_w_val = st.number_input(label=get_text_local("module_capacity_wp_label","Leistung (Wp)"), min_value=0.0, value=float(product_data_for_manual_form.get('capacity_w', 0.0)), step=1.0, key=f"{form_key_manual_prod_ui}_cap_w_man")
                p_efficiency_percent_val = st.number_input(label=get_text_local("module_efficiency_label","Wirkungsgrad (%)"), min_value=0.0,max_value=100.0, value=float(product_data_for_manual_form.get('efficiency_percent', 0.0)),step=0.01,format="%.2f", key=f"{form_key_manual_prod_ui}_eff_mod_man")
                ...
                p_power_kw_val = st.number_input(label=get_text_local("inverter_power_ac_kw_label","Nennleistung AC (kW)"), min_value=0.0, value=float(product_data_for_manual_form.get('power_kw', 0.0)), step=0.1, key=f"{form_key_manual_prod_ui}_power_kw_inv_man")
                p_efficiency_percent_val = st.number_input(label=get_text_local("inverter_max_efficiency_label","Max. Wirkungsgrad (%)"), min_value=0.0,max_value=100.0, value=float(product_data_for_manual_form.get('efficiency_percent', 0.0)),step=0.01,format="%.2f", key=f"{form_key_manual_prod_ui}_eff_inv_man")
            elif p_category_form == 'Batteriespeicher':
                p_storage_power_kw_val = st.number_input(label=get_text_local("storage_usable_capacity_kwh_label","Nutzbare Kapazität (kWh)"),min_value=0.0,value=float(product_data_for_manual_form.get('storage_power_kw', 0.0)),step=0.1, key=f"{form_key_manual_prod_ui}_storage_cap_man")
                p_power_kw_val = st.number_input(label=get_text_local("storage_max_charge_discharge_power_kw_label","Max. Lade-/Entladeleistung (kW)"),min_value=0.0,value=float(product_data_for_manual_form.get('power_kw', 0.0)),step=0.1, key=f"{form_key_manual_prod_ui}_storage_power_man")
                p_max_cycles_val = st.number_input(label=get_text_local("storage_cycles_manufacturer_label","Zyklen (Herstellerangabe)"),min_value=0,value=int(product_data_for_manual_form.get('max_cycles',0)),step=100, key=f"{form_key_manual_prod_ui}_max_cycles_man")
        ...
        if submitted_manual_product_form_btn:
            if not p_model_name_form.strip() or not p_category_form: st.error(get_text_local("product_error_model_category_required","Modellname und Kategorie sind Pflichtfelder."))
            else:
                product_data_to_save_db = {"model_name": p_model_name_form.strip(), "category": p_category_form, "brand": p_brand_form.strip(), "price_euro": p_price_form, "additional_cost_netto": p_add_cost_form, "warranty_years": p_warranty_form, "description": p_description_form_val.strip(), "image_base64": current_product_image_b64_form, "datasheet_link_db_path": current_datasheet_link }
                if p_category_form == 'Modul': product_data_to_save_db.update({"capacity_w": p_capacity_w_val, "efficiency_percent": p_efficiency_percent_val, "length_m": p_length_m_val, "width_m": p_width_m_val, "weight_kg": p_weight_kg_val})
                elif p_category_form == 'Wechselrichter': product_data_to_save_db.update({"power_kw": p_power_kw_val, "efficiency_percent": p_efficiency_percent_val})
                elif p_category_form == 'Batteriespeicher': product_data_to_save_db.update({"storage_power_kw": p_storage_power_kw_val, "power_kw": p_power_kw_val, "max_cycles": p_max_cycles_val})
                if uploaded_product_image_manual_file_form:
                    if uploaded_product_image_manual_file_form.size <= 2*1024*1024: product_data_to_save_db["image_base64"] = base64.b64encode(uploaded_product_image_manual_file_form.getvalue()).decode('utf-8')
                    else: st.error(get_text_local("product_error_image_too_large","Produktbild zu groß (max. 2MB). Nicht gespeichert."))
                datasheet_content_bytes_to_write, original_datasheet_filename_to_write = None, None
                if uploaded_datasheet_pdf_file_form:
                    if uploaded_datasheet_pdf_file_form.size <= 5*1024*1024: datasheet_content_bytes_to_write = uploaded_datasheet_pdf_file_form.getvalue(); original_datasheet_filename_to_write = uploaded_datasheet_pdf_file_form.name
                    else: st.error(get_text_local("product_error_datasheet_too_large","Datenblatt-PDF zu groß (max. 5MB). Nicht hochgeladen."))
                if st.session_state.product_to_edit_id_manual: 
                    ...
                    if datasheet_content_bytes_to_write and original_datasheet_filename_to_write:
                        ...
                        if old_ds_path: 
                            ...
                            if os.path.exists(full_old_ds_path):
                                try:
                                    os.remove(full_old_ds_path); parent_dir_old_ds = os.path.dirname(full_old_ds_path)
                                    if os.path.exists(parent_dir_old_ds) and not os.listdir(parent_dir_old_ds):
                                        try: os.rmdir(parent_dir_old_ds)
                                        except OSError as e_rmdir_old: st.warning(f"Konnte leeres altes Datenblatt-Verzeichnis nicht löschen: {e_rmdir_old}")
                                except OSError as e_del_old: st.warning(f"Altes Datenblatt konnte nicht gelöscht werden: {e_del_old}")
                        prod_specific_dir = os.path.join(PRODUCT_DATASHEETS_BASE_DIR_ADMIN, str(product_id_for_files))
                        if not os.path.exists(prod_specific_dir): os.makedirs(prod_specific_dir)
                        safe_filename = "".join(c if c.isalnum() or c in ('.', '-', '_') else '_' for c in original_datasheet_filename_to_write)
                        timestamp = datetime.now().strftime("%Y%m%d%H%M%S"); final_safe_filename_ds = f"{os.path.splitext(safe_filename)[0]}_{timestamp}{os.path.splitext(safe_filename)[1]}"
                        disk_path = os.path.join(prod_specific_dir, final_safe_filename_ds)
                        with open(disk_path, "wb") as f: f.write(datasheet_content_bytes_to_write)
                    ...
                    if update_product_func(product_id_for_files, product_data_to_save_db): 
                        st.success(get_text_local("product_success_updated","Produkt erfolgreich aktualisiert.")); st.session_state.selected_page_key_sui = "admin"; st.rerun() 
                    else: st.error(get_text_local("product_error_updating","Fehler beim Aktualisieren des Produkts."))
                else: 
                    product_data_for_add = product_data_to_save_db.copy()
                    if "datasheet_link_db_path" in product_data_for_add: del product_data_for_add["datasheet_link_db_path"] 
                    ...
                    if new_product_id:
                        ...
                        if datasheet_content_bytes_to_write and original_datasheet_filename_to_write:
                            ...
                            if update_product_func(new_product_id, {"datasheet_link_db_path": relative_ds_path}): st.info(get_text_local("product_info_datasheet_saved","Produktdatenblatt gespeichert..."))
                            else: st.error(get_text_local("product_error_datasheet_path","Produkt angelegt, aber Fehler beim Speichern des Datenblatt-Pfads."))
                        st.session_state.product_to_edit_id_manual = new_product_id 
                        st.session_state.selected_page_key_sui = "admin"; st.rerun()
                    else: st.error(get_text_local("product_error_adding","Fehler beim Anlegen des Produkts. Modellname bereits vorhanden?"))
        if st.session_state.product_to_edit_id_manual and product_data_for_manual_form.get("datasheet_link_db_path"):
            ...
            if st.button(get_text_local("product_button_delete_current_datasheet","Aktuelles Datenblatt löschen"), key=f"delete_datasheet_prod_btn_man_{st.session_state.product_to_edit_id_manual}_v15_final_key"): 
                datasheet_to_delete_path_val = product_data_for_manual_form.get("datasheet_link_db_path"); full_path_to_delete_val = os.path.join(PRODUCT_DATASHEETS_BASE_DIR_ADMIN, datasheet_to_delete_path_val); file_deleted_fs_val = False
                if os.path.exists(full_path_to_delete_val):
                    try:
                        os.remove(full_path_to_delete_val); parent_dir_ds_val = os.path.dirname(full_path_to_delete_val)
                        if os.path.exists(parent_dir_ds_val) and not os.listdir(parent_dir_ds_val): os.rmdir(parent_dir_ds_val)
                        file_deleted_fs_val = True
                    except OSError as e_del_ds_f: st.error(f"Fehler beim Löschen der Datenblatt-Datei: {e_del_ds_f}")
                else: st.warning(get_text_local("product_warning_datasheet_not_found","Datenblatt-Datei nicht auf dem Server gefunden...")); file_deleted_fs_val = True 
                if file_deleted_fs_val:
                    if update_product_func(st.session_state.product_to_edit_id_manual, {"datasheet_link_db_path": None}): 
                        st.success(get_text_local("product_success_datasheet_link_removed","Datenblatt-Verknüpfung entfernt.")); st.session_state.selected_page_key_sui = "admin"; st.rerun()
                    else: st.error(get_text_local("product_error_removing_datasheet_path","Fehler beim Entfernen des Datenblatt-Pfads aus der DB."))
        if st.session_state.product_to_edit_id_manual:
            if st.button(get_text_local("admin_finish_edit_product_button", "Produktbearbeitung abschließen/Neues Produkt"), key="finish_edit_prod_btn_manual_man_v15_final_key_end"): 
                st.session_state.product_to_edit_id_manual = None; st.session_state.selected_page_key_sui = "admin"; st.rerun()
    st.markdown("---"); st.subheader(get_text_local("admin_current_products_header_display", "Produkte in Datenbank (gefiltert)"))
    selected_category_filter_disp_list = st.selectbox(get_text_local("admin_select_category_label_display", "Angezeigte Produktkategorie"), options=[get_text_local("all_categories", "Alle")] + product_categories_manual_list, key="product_cat_select_display_list_man_v15_final_key_filter") 
    ...
    if not all_products_list_display_val: st.info(get_text_local("product_info_no_products_for_filter","Keine Produkte zum Anzeigen gefunden für die aktuelle Filterauswahl."))
    else:
        list_cols_h = st.columns([0.4, 2, 0.8, 0.8, 1, 0.8, 0.4, 0.4]); list_headers = ["ID", "Modell", "Bild", "Datenblatt?", "Kategorie", "Preis (€)", "📝", "🗑️"]
        for c, h in zip(list_cols_h, list_headers): c.markdown(f"**{h}**")
        for i_list, prod_item_in_list in enumerate(all_products_list_display_val):
            prod_id_in_list = prod_item_in_list.get('id'); key_suffix_in_list = prod_id_in_list if prod_id_in_list is not None else f"idx_prod_list_man_final_key_admin_item_{i_list}_v15_final" 
            list_cols_r = st.columns([0.4, 2, 0.8, 0.8, 1, 0.8, 0.4, 0.4])
            list_cols_r[0].text(str(prod_id_in_list) if prod_id_in_list is not None else "N/A"); list_cols_r[1].text(f"{prod_item_in_list.get('brand','') or ''} {prod_item_in_list.get('model_name','') or ''}".strip())
            prod_img_b64_list_view = prod_item_in_list.get('image_base64')
            if prod_img_b64_list_view:
                try: list_cols_r[2].image(base64.b64decode(prod_img_b64_list_view), width=40)
                except Exception: list_cols_r[2].caption("err")
            else: list_cols_r[2].caption("-")
            list_cols_r[3].caption("Ja" if prod_item_in_list.get("datasheet_link_db_path") else "Nein"); list_cols_r[4].caption(prod_item_in_list.get('category',''))
            price_euro_val = prod_item_in_list.get('price_euro'); price_display_val = 0.0
            try: price_display_val = float(price_euro_val if price_euro_val is not None else 0.0)
            except (ValueError, TypeError): pass
            list_cols_r[5].caption(f"{price_display_val:.2f}")
            if list_cols_r[6].button("📝", key=f"edit_prod_list_btn_man_final_key_admin_item_{key_suffix_in_list}_v15_final", help="Produkt bearbeiten"): 
                if prod_id_in_list is not None: st.session_state.product_to_edit_id_manual = prod_id_in_list; st.session_state.selected_page_key_sui = "admin"; st.rerun()
            confirm_del_key_prod_item_list_final = f"confirm_del_prod_item_list_man_final_key_admin_item_{key_suffix_in_list}_v15_final"
            if list_cols_r[7].button("🗑️", key=f"delete_prod_list_btn_man_final_key_admin_item_{key_suffix_in_list}_v15_final", help="Produkt löschen"):
                if prod_id_in_list is not None:
                    if st.session_state.get(confirm_del_key_prod_item_list_final, False):
                        if delete_product_func(prod_id_in_list):
                            ...
                            if confirm_del_key_prod_item_list_final in st.session_state: del st.session_state[confirm_del_key_prod_item_list_final]
                            if st.session_state.product_to_edit_id_manual == prod_id_in_list: st.session_state.product_to_edit_id_manual = None
                            st.session_state.selected_page_key_sui = "admin"; st.rerun()
                        else: st.error(get_text_local("product_error_deleting_param","Fehler Löschen Produkt '{model_name}'.").format(model_name=prod_item_in_list.get('model_name')))
                    else:
                        st.session_state[confirm_del_key_prod_item_list_final] = True; st.warning(get_text_local("product_confirm_delete_param","Sicher Produkt '{model_name}' löschen? Erneut klicken.").format(model_name=prod_item_in_list.get('model_name'))); st.session_state.selected_page_key_sui = "admin"; st.rerun()
            st.divider()
...
def render_general_settings_extended(load_admin_setting_func: Callable, save_admin_setting_func: Callable):
    ...
    with st.form("global_constants_form_v15_final"): 
        col_gc1, col_gc2 = st.columns(2)
        with col_gc1: # Text-Keys für Labels verwenden
            new_vat_rate = st.number_input(label=get_text_local("vat_rate_percent", "Standard MwSt. (%)"), value=float(current_global_constants.get('vat_rate_percent', 19.0)), key="gc_vat_v15_final")
            new_degradation = st.number_input(label=get_text_local("annual_module_degradation_percent", "Leistungsdegradation... (% pro Jahr)"), value=float(current_global_constants.get('annual_module_degradation_percent', 0.5)), key="gc_degradation_v15_final", format="%.2f")
            new_maintenance_fixed = st.number_input(label=get_text_local("maintenance_fixed_eur_pa", "Jährliche Wartungspauschale (€, netto)"), value=float(current_global_constants.get('maintenance_fixed_eur_pa', 50.0)), key="gc_maint_fixed_v15_final", format="%.2f")
        with col_gc2:
            new_inflation = st.number_input(label=get_text_local("inflation_rate_percent", "Inflationsrate... (% p.a.)"), value=float(current_global_constants.get('inflation_rate_percent', 2.0)), key="gc_inflation_v15_final", format="%.2f")
            new_maintenance_increase = st.number_input(label=get_text_local("maintenance_increase_percent_pa", "Jährliche Steigerung Wartungskosten (% p.a.)"), value=float(current_global_constants.get('maintenance_increase_percent_pa', 2.0)), key="gc_maint_increase_v15_final", format="%.2f")
            new_maint_var = st.number_input(label=get_text_local("maintenance_variable_eur_per_kwp_pa", "Variable Wartungskosten (€ pro kWp p.a., netto)"), value=float(current_global_constants.get('maintenance_variable_eur_per_kwp_pa', 5.0)), key="gc_maint_var_v15_final", format="%.2f")
            new_alt_invest_interest = st.number_input(label=get_text_local("alternative_investment_interest_rate_percent", "Vergleichszinssatz Alternativanlage (% p.a.)"), value=float(current_global_constants.get('alternative_investment_interest_rate_percent', 3.0)), key="gc_alt_invest_v15_final", format="%.2f")
        st.markdown("---")
        st.subheader(get_text_local("admin_yield_settings_header", "Ertragseinstellungen für PV-Anlage"))
        col_yield1, col_yield2 = st.columns(2)
        with col_yield1: new_global_yield_adj = st.number_input(label=get_text_local("global_yield_adjustment_percent", "Globale Ertragsanpassung (%)"), value=float(current_global_constants.get('global_yield_adjustment_percent', 0.0)), key="gc_yield_adj_v15_final", format="%.1f", help="Ein positiver Wert erhöht...")
        with col_yield2: new_ref_yield_pr = st.number_input(label=get_text_local("reference_specific_yield_pr", "Referenz-Spezialertrag für PR (kWh/kWp/a)"), value=float(current_global_constants.get('reference_specific_yield_pr', 1100.0)), key="gc_ref_yield_pr_v15_final", format="%.0f", help="Wird für die Performance Ratio Berechnung verwendet.")
        ...
        for ori in orientations_ordered:
            ...
            for i_tilt, tilt_val_str_loop in enumerate(tilts_ordered):
                ...
                updated_specific_yields_map[ori][tilt_val_str_loop] = row_cols[i_tilt+1].number_input(label=f"{ori}-{tilt_val_str_loop}", value=current_val_for_field, min_value=0.0, max_value=2000.0, step=10.0, format="%.0f", key=f"gc_yield_{ori}_{tilt_val_str_loop}_v15_final", label_visibility="collapsed")
        ...
        with col_my1: new_sim_period = col_my1.number_input(label=get_text_local("simulation_period_years", "Simulationsdauer (Jahre)"), value=int(current_global_constants.get('simulation_period_years', 20)), min_value=1, max_value=50, step=1, key="gc_sim_period_v15_final")
        with col_my2: new_elec_price_increase = col_my2.number_input(label=get_text_local("electricity_price_increase_annual_percent", "Strompreissteigerung (% p.a.)"), value=float(current_global_constants.get('electricity_price_increase_annual_percent', 3.0)), min_value=0.0, max_value=20.0, step=0.1, format="%.2f", key="gc_elec_increase_v15_final")
...
def render_price_matrix(
    ...
    parse_csv_func_from_calculations: Callable[[Union[str, io.StringIO], List[str]], Optional[pd.DataFrame]],
    parse_excel_func_local_admin: Callable[[Optional[bytes], List[str]], Optional[pd.DataFrame]]
):
    ...
    uploaded_file_excel = st.file_uploader(get_text_local("admin_upload_xlsx_label", "Excel-Datei (.xlsx) hochladen"), type=["xlsx"], key="price_matrix_upload_widget_xlsx_v15_final")
    ...
    if st.session_state.get('parsed_excel_df_for_preview_admin') is not None:
        ...
        if st.button(get_text_local("admin_save_price_matrix_button_xlsx", "...Excel...speichern"), key="save_uploaded_price_matrix_xlsx_btn_v15_final"):
    ...
    if current_price_matrix_excel_bytes_from_db and isinstance(current_price_matrix_excel_bytes_from_db, bytes):
        ...
        if parsed_stored_df_pm_excel is not None and not parsed_stored_df_pm_excel.empty:
            ...
            if st.button(get_text_local("admin_delete_saved_price_matrix_button_xlsx", "Excel Preis-Matrix löschen"), key="delete_price_matrix_excel_final_v15_final"):
    ...
    uploaded_file_csv = st.file_uploader(get_text_local("admin_upload_csv_label", "CSV-Datei hochladen"), type=["csv"], key="price_matrix_upload_widget_csv_v15_final")
    ...
    if st.session_state.get('parsed_csv_df_for_preview_admin') is not None:
        ...
        if st.button(get_text_local("admin_save_price_matrix_button_csv", "...CSV...speichern"), key="save_uploaded_price_matrix_csv_btn_v15_final"):
    ...
    if current_price_matrix_csv_from_db and isinstance(current_price_matrix_csv_from_db, str) and current_price_matrix_csv_from_db.strip():
        ...
        if parsed_stored_df_pm_csv is not None and not parsed_stored_df_pm_csv.empty:
            ...
            if st.button(get_text_local("admin_delete_saved_price_matrix_button_csv", "...CSV...löschen"), key="delete_price_matrix_csv_final_v15_final"):
            ...
            st.warning("Gespeicherte CSV Preis-Matrix ungültig."); st.text_area("Rohdaten CSV (max. 500 Z.)", value=current_price_matrix_csv_from_db[:500]+"..." if len(current_price_matrix_csv_from_db) > 500 else current_price_matrix_csv_from_db, height=100, disabled=True, key="raw_db_csv_preview_v15_final")
...
def render_tariff_management(load_admin_setting_func: Callable, save_admin_setting_func: Callable):
    ...
    form_key_parts = "tariff_form_parts_v15_final" 
    st.markdown(f"#### {get_text_local('admin_partial_feed_in_header', 'Teileinspeisung')}")
    with st.form(key=form_key_parts):
        rendered_tariffs_parts_ui, final_valid_parts_form_check = manage_tariff_list_ui(current_feed_in_tariffs_block['parts'], "parts_cat_v15_final")
    ...
    st.markdown("---")
    form_key_full = "tariff_form_full_v15_final" 
    st.markdown(f"#### {get_text_local('admin_full_feed_in_header', 'Volleinspeisung')}")
    with st.form(key=form_key_full):
        rendered_tariffs_full_ui, final_valid_full_form_check = manage_tariff_list_ui(current_feed_in_tariffs_block['full'], "full_cat_v15_final")
...
def render_visualization_settings(load_admin_setting_func: Callable, save_admin_setting_func: Callable ):
    ...
    with st.form("visualization_settings_form_v15_final"):
        ...
        selected_palette = st.selectbox(get_text_local("admin_viz_default_palette_label", "Standard-Farbpalette..."), options=plotly_palettes, index=plotly_palettes.index(selected_palette_val) if selected_palette_val in plotly_palettes else 0, key="viz_default_palette_select_v15_final", help=get_text_local("admin_viz_default_palette_help", "..."))
        primary_color = st.color_picker(get_text_local("admin_viz_primary_color_label", "Primäre Diagrammfarbe"), value=viz_settings.get("primary_chart_color", default_viz_settings_fallback.get("primary_chart_color")), key="viz_primary_color_picker_v15_final")
        secondary_color = st.color_picker(get_text_local("admin_viz_secondary_color_label", "Sekundäre Diagrammfarbe"), value=viz_settings.get("secondary_chart_color", default_viz_settings_fallback.get("secondary_chart_color")), key="viz_secondary_color_picker_v15_final")
        st.markdown(f"---<br>**{get_text_local('admin_viz_font_settings_header', 'Schriftart-Einstellungen')}**")
        font_family = st.text_input(get_text_local("admin_viz_font_family_label", "Schriftfamilie..."), value=viz_settings.get("chart_font_family", default_viz_settings_fallback.get("chart_font_family")), key="viz_font_family_input_v15_final")
        c1f, c2f, c3f = st.columns(3) 
        font_size_title = c1f.number_input(get_text_local("admin_viz_font_size_title_label", "Schriftgröße Titel"), min_value=8, max_value=30, value=int(viz_settings.get("chart_font_size_title", default_viz_settings_fallback.get("chart_font_size_title"))), key="viz_font_size_title_num_v15_final")
        font_size_axis = c2f.number_input(get_text_local("admin_viz_font_size_axis_label", "Schriftgröße Achsenbeschr."), min_value=6, max_value=24, value=int(viz_settings.get("chart_font_size_axis_label", default_viz_settings_fallback.get("chart_font_size_axis_label"))), key="viz_font_size_axis_num_v15_final")
        font_size_tick = c3f.number_input(get_text_local("admin_viz_font_size_tick_label", "Schriftgröße Achsenticks"), min_value=5, max_value=20, value=int(viz_settings.get("chart_font_size_tick_label", default_viz_settings_fallback.get("chart_font_size_tick_label"))), key="viz_font_size_tick_num_v15_final")
...
def render_advanced_settings(load_admin_setting_func: Callable, save_admin_setting_func: Callable ):
    ...
    edited_locale_json_string = st.text_area(get_text_local("admin_locale_json_editor_label", "Texte (JSON-Editor)"), value=locale_json_string, height=300, key="locale_json_editor_final_v15_final")
    if st.button(get_text_local("admin_save_locale_button", "Lokalisierungstexte speichern"), key="save_locale_btn_final_v15_final"):
      try:
    ...
    with st.form("debug_settings_form_final_v15_final"):
        app_debug_mode_new_val = st.checkbox(get_text_local("admin_enable_debug_mode_label", "App-weiten Debug-Modus aktivieren..."),value=current_debug_mode_for_display,key="app_debug_mode_checkbox_final_key_v15_final", help=get_text_local("admin_enable_debug_mode_help", "..."))
    ...
    if st.checkbox(get_text_local("admin_show_reset_options_label", "Optionen zum Zurücksetzen anzeigen..."), key="show_reset_options_final_v15_final"):
        ...
        user_confirm_text = st.text_input(f"Geben Sie exakt '{confirm_text_reset}' ein...", key="confirm_delete_text_input_final_v15_final")
        if st.button(get_text_local("admin_reset_app_data_button", "🚨 App-Daten jetzt endgültig zurücksetzen"), key="reset_all_app_data_btn_final_v15_final", type="primary"):
...
def render_pdf_design_settings(load_admin_setting_func: Callable, save_admin_setting_func: Callable):
    ...
    if _get_company_by_id_safe and callable(_get_company_by_id_safe) and active_company_id: # Verwende _get_company_by_id_safe
    ...
    with col_pdf_info:
        ...
        for field_key, label_key_suffix in [("name","name"), ("street","address1"), ("zip_code","address2_zip"), ("city","address2_city"), ("phone","phone"), ("email","email"), ("website","website"), ("tax_id","tax_id"), ("commercial_register","commercial_register")]:
            ...
            st.text_input(get_text_local(f"admin_pdf_company_{label_key_suffix}", field_key.replace("_"," ").title()), value=val_disp, disabled=True, key=f"pdf_disp_ci_{field_key}_v15_final")
    ...
    with st.form("pdf_design_form_v15_final"):
      primary_color = st.color_picker(get_text_local("admin_pdf_primary_color_label", "PDF Haupt-/Akzentfarbe"), value=current_design_settings.get('primary_color', PDF_DESIGN_SETTINGS_DEFAULT['primary_color']), key="pdf_primary_color_v15_final")
      secondary_color = st.color_picker(get_text_local("admin_pdf_secondary_color_label", "PDF Sekundärfarbe"), value=current_design_settings.get('secondary_color', PDF_DESIGN_SETTINGS_DEFAULT['secondary_color']), key="pdf_secondary_color_v15_final")
...
def manage_templates_local(template_type_key: str, template_list_setting_key: str, item_name_label_key: str, item_content_label_key: Optional[str] = None, is_image_template: bool = False ):
    ...
    form_key_mt = f"{template_type_key}_form_mt_local_tpl_v15_final"
    edit_mode_session_key = f"edit_mode_template_{template_type_key}_v15_final"
    edit_index_session_key = f"edit_index_template_{template_type_key}_v15_final"
    ...
    selected_template_display_name = st.selectbox(get_text_local("admin_select_template_to_edit_or_add_new", "Vorlage bearbeiten..."), options=options_for_select, key=f"select_or_add_template_{template_type_key}_v15_final_select", index=current_selection_index)
    ...
    with st.form(form_key_mt, clear_on_submit=False):
        ...
        new_template_name = st.text_input(get_text_local(item_name_label_key, "Vorlagenname"), value=current_name_val, key=f"{template_type_key}_name_input_mt_v15_final_form")
        new_template_data_b64_for_save = None; new_template_content_input = "" 
        if is_image_template:
            uploaded_image_file = st.file_uploader(get_text_local("admin_upload_title_image", "Bild hochladen..."), type=["png", "jpg", "jpeg"], key=f"{template_type_key}_upload_fu_mt_v15_final_form")
        ...
        else: new_template_content_input = st.text_area(get_text_local(item_content_label_key or "admin_template_content_label", "Inhalt..."), value=current_content_val, height=200, key=f"{template_type_key}_content_ta_mt_v15_final_form")
    ...
    if st.session_state[edit_mode_session_key] and st.session_state[edit_index_session_key] != -1:
        if 0 <= st.session_state[edit_index_session_key] < len(templates):
            delete_button_key = f"delete_template_{template_type_key}_v15_final_outer_btn"
            confirm_delete_session_key = f"confirm_delete_tpl_{template_type_key}_{st.session_state[edit_index_session_key]}_v15_final_conf"
...
def render_api_key_settings(load_admin_setting_func: Callable, save_admin_setting_func: Callable):
    ...
    current_api_keys_values_for_display = {key_name: load_admin_setting_func(key_name, "LEER_DEFAULT") for key_name in api_keys_to_manage.keys()} # .keys() hinzugefügt
    with st.form("api_keys_form_v15_final"):
        new_api_key_inputs = {}
        for key_name, key_label_text in api_keys_to_manage.items():
            ...
            new_api_key_inputs[key_name] = st.text_input(key_label_text, value="", type="password", help=f"{get_text_local('api_key_current_value_notice', 'Aktuell...')}: {display_value_placeholder}. {get_text_local('api_key_input_help', 'Geben Sie hier einen neuen Schlüssel ein...')}", key=f"api_key_input_{key_name}_v15_final")
...
def manage_tariff_list_ui(tariff_list_input: List[Dict[str, Any]], form_element_key_suffix: str) -> Tuple[List[Dict[str, Any]], bool]:
    ...
    for i, tariff_entry_db_like in enumerate(actual_tariff_list_for_ui):
        cols_tariff_ui = st.columns([2, 2, 2, 1]); key_base = f"tariff_entry_{form_element_key_suffix}_{i}_v15_final_ui"
...
def render_admin_panel(
    ...
    **kwargs: Any 
    ...
    # KORREKTUR: Sicherstellen, dass st.session_state['_admin_panel_texts'] ein Dictionary ist
    actual_texts_dict_for_session: Dict[str, str]
    if isinstance(texts, dict):
        actual_texts_dict_for_session = texts
    elif isinstance(texts, tuple): 
        st.warning(f"ADMIN_PANEL WARNUNG: Der 'texts'-Parameter wurde als Tupel übergeben. Versuche Konvertierung zu Dict. Dies deutet auf ein Problem in 'gui.py' oder 'locales.py' hin.")
        try:
            actual_texts_dict_for_session = dict(texts) 
            if not actual_texts_dict_for_session and texts: 
                 st.warning("ADMIN_PANEL WARNUNG: Konnte Tupel-Parameter 'texts' nicht sinnvoll in Dict umwandeln. Verwende leeres Text-Dict.")
                 actual_texts_dict_for_session = {}
        except (TypeError, ValueError):
            st.error(f"ADMIN_PANEL FEHLER: Konnte Tupel-Parameter 'texts' nicht in Dict umwandeln. Verwende leeres Text-Dict.")
            actual_texts_dict_for_session = {} 
    else:
    ...
    if not admin_tab_labels_definition: 
        st.error("FEHLER: Keine Admin-Tabs zum Anzeigen vorhanden. Überprüfen Sie ADMIN_TAB_KEYS_DEFINITION_GLOBAL und Textdefinitionen.")
        return
    ...
    for i, tab_key_loop in enumerate(admin_tab_keys_definition): 
        if i < len(admin_tabs_rendered_ui): 
        ...
        else: st.error(f"Fehler: Index {i} außerhalb des Bereichs für admin_tabs_rendered_ui...")

# Änderungshistorie
# ... (vorherige Einträge) ...
# 2025-06-03, Gemini Ultra: Sicherstellung, dass `st.session_state['_admin_panel_texts']` in `render_admin_panel`
#                           immer ein Dictionary ist, selbst wenn der `texts`-Parameter fehlerhaft ist.
#                           Alle Tab-Render-Funktionen (render_company_crud_tab, render_product_management, etc.)
#                           so angepasst, dass sie keinen `texts`-Parameter mehr erwarten und stattdessen
#                           intern `get_text_local` für alle Textabrufe verwenden.
#                           Lambdas in `tab_functions_map` übergeben `texts` nicht mehr.
#                           Widget-Keys auf _v15_final aktualisiert.