# database.py (Schema Version 13 - Spaltennamen und last_modified korrigiert)
import sqlite3
import os
import traceback
import json
...
def reset_database() -> bool:
    try:
        ...
        return False# database.py (Schema Version 13 - Spaltennamen und last_modified korrigiert)
import sqlite3
import os
import traceback
import json
...
def save_admin_setting(key: str, value: Any) -> bool:
    ...
    try:
        cursor = conn.cursor()
        value_to_save = json.dumps(value) if isinstance(value, (dict, list)) else int(value) if isinstance(value, bool) else value
        ...
        """
        # Korrekte Parameterübergabe, besonders für None-Werte
...
# Restliche Funktionen (list_companies, get_company, etc.) bleiben unverändert
# ... (fügen Sie hier den Rest Ihres database.py Codes ein)
...
if __name__ == '__main__':
    print("--- Starte database.py direkten Testlauf ---")
    if os.path.exists(DB_PATH): 
        print(f"Lösche existierende DB-Datei: {DB_PATH}")
        try: os.remove(DB_PATH)
        except Exception as e_del: print(f"Konnte DB nicht löschen: {e_del}")
    if os.path.exists(COMPANY_DOCS_BASE_DIR): 
        import shutil
        print(f"Lösche Firmen-Doc-Verzeichnis: {COMPANY_DOCS_BASE_DIR}")
        try: shutil.rmtree(COMPANY_DOCS_BASE_DIR)
        except Exception as e_del_dir: print(f"Konnte Verzeichnis nicht löschen: {e_del_dir}")
    init_db()
    print("\n--- Teste Admin Settings ---")
    save_admin_setting("test_setting_main", "test_value_main"); 
    loaded_setting = load_admin_setting('test_setting_main'); 
    print(f"Load test_setting_main: {loaded_setting} -> {'✅ OK' if loaded_setting == 'test_value_main' else '❌ FEHLGESCHLAGEN'}")
    print("\n--- Teste Firmenanlage ---")
    comp1_data = {"name": "SolarTech GmbH ", "email": "info@solartech.de", "is_default": 1}
    comp1_id = add_company(comp1_data)
    if comp1_id:
        print(f"Firma 1 '{comp1_data['name']}' angelegt mit ID: {comp1_id} -> ✅ OK")
        active_comp = get_active_company()
        if active_comp and active_comp['id'] == comp1_id:
            print(f"Firma '{active_comp['name']}' ist aktiv und Standard. -> ✅ OK")
        else:
            print(f"Fehler: Aktive Firma ist {active_comp} -> ❌ FEHLGESCHLAGEN")
    else:
        print(f"Fehler beim Anlegen von Firma 1 '{comp1_data['name']}' -> ❌ FEHLGESCHLAGEN")
    print("\n--- Teste Duplikatsprüfung (case-insensitive) ---")
    comp3_data_dup = {"name": "solartech gmbh", "email": "dup@solartech.de"}
    comp3_id = add_company(comp3_data_dup)
    if comp3_id is None:
        print(f"Duplikatsversuch für '{comp3_data_dup['name']}' korrekt abgefangen. -> ✅ OK")
    else:
        print(f"FEHLER: Duplikat '{comp3_data_dup['name']}' wurde nicht abgefangen, ID: {comp3_id} -> ❌ FEHLGESCHLAGEN")
    print("\n--- Teste Duplikatsprüfung (exakt) ---")
    # Erneut versuchen, die erste Firma exakt anzulegen (nachdem strip() angewendet wurde)
    comp4_data_exact_dup = {"name": "SolarTech GmbH", "email": "info@solartech.de"} 
    comp4_id = add_company(comp4_data_exact_dup)
    if comp4_id is None:
         print(f"Exakter Duplikatsversuch für '{comp4_data_exact_dup['name']}' korrekt abgefangen. -> ✅ OK")
    else:
        print(f"FEHLER: Exaktes Duplikat '{comp4_data_exact_dup['name']}' wurde nicht korrekt abgefangen, ID: {comp4_id} -> ❌ FEHLGESCHLAGEN")
    print("\n--- Auflistung aller Firmen ---")
    all_companies = list_companies()
    for c in all_companies: print(f"  ID: {c['id']}, Name: '{c['name']}', Default: {c['is_default']}") # Name in '' zur Sichtbarmachung von Leerzeichen
    print("\n--- Testlauf database.py Ende ---")